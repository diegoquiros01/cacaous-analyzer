...
function renderMatrix(){
  ...
  activeFields.forEach(f => {
    ...
    otherDocs.forEach(d => {
      ...
      // Fumigation/phyto certs are per-lot: their bag count, weights are for a single lot, not the full shipment.
      // Don't compare these fields against the BL total — show as "—" instead of "DIFF".
      const dt = (d.docType||'').toLowerCase();
      const fn = (d._filename||'').toLowerCase();
      const isPerLotDoc = dt.includes('fumig') || dt.includes('gas clearance') || dt.includes('quarantine')
